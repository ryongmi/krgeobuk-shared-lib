...
export function ExposeSourceEmail(): PropertyDecorator {
  const propertyData = {
    example: 'auth@naver.com',
    description: 'User A (새로운 OAuth 계정 소유자) 이메일',
  };

  return applyDecorators(SwaggerApiProperty(propertyData), Expose());
}

export function ExposeTargetEmail(): PropertyDecorator {
  const propertyData = {
    example: 'auth2@naver.com',
    description: 'User B (기존 계정 소유자) 이메일',
  };
